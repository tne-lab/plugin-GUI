...
namespace PhaseCalculator
{
    ...
    void ReverseStack::unwrapAndCopy(double* dest) const
    {
        int length = size();
    ...
    void ActiveChannelInfo::update()
    {
        ...
        sharedHistory.apply([=](Array<double>& arr)
        {
            arr.resize(newHistorySize);
        });
    ...
    bool Node::disable()
    {
        ...
        // once we're sure there's no more ARModeler or sharedHistory activity in the thread...
    ...
    void Node::calcVisPhases(ActiveChannelInfo* acInfo, juce::int64 sdbEndTs)
    {
        ...
        if (!visTsBuffer.empty() && visTsBuffer.front() <= maxTs)
        {
            ...
            acInfo->history.unwrapAndCopy(wpHilbert);
            ...
            {
                visPhaseBuffer.push(tempPhaseBuffer.front());
                tempPhaseBuffer.pop();
            }
        }
    }
